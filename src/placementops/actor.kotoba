(ns placementops.actor
  "PlacementOpsActor — the ISCO-08 3333 community employment-agents-
  and-contractors actor as a `langgraph.graph/state-graph` (ADR-
  2607121000 / CLAUDE.md Actors section). One graph run = one
  placement-coordination operation request (intake → advise → govern
  ...
  The unconditional invariant: the PlacementAdvisor can never directly
  commit a record the PlacementOpsGovernor refuses — every
  commit-record! call is gated behind `:decide` — and NO commit path
  can ever finalize a hiring decision, a candidate-rejection decision,
  or any placement decision that could constitute discriminatory
  screening (permanently out of scope, see `placementops.governor`)."
...
(defn build-graph
  "Build a compiled PlacementOpsActor graph. `store` implements
  `placementops.store/Store`. `advisor` implements
  `placementops.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:candidate-id (:candidate-id proposal)
                                    :employer-id (:employer-id proposal)
                                    :op (:op proposal)
                                    :payload proposal}]
